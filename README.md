...
NodeCtl 提供简洁直观的 Web 界面与稳定的 Golang 后台，帮助你集中管理**自建节点**和**机场订阅**。兼顾易用性与高度可定制的分流组网能力。
...
### ☁️ Cloudflare 集成
- **Token 管理**：Token 权限验证（Read/Edit 区分）、安全保存与记录
- **CF Tunnel 一键管理**：自动下载 cloudflared、创建 Tunnel、绑定 DNS、一键部署、启停控制、实时日志与状态查看，程序启动后自动拉起已配置的 Tunnel
- **自动签发 SSL**：通过 Cloudflare API 自动签发与续签 SSL 证书，支持热重载无重启切换 HTTP/HTTPS

### ⚙️ 系统与架构
- **零依赖**：纯 Golang 后端，嵌入式 SQLite (GORM)，无需额外安装任何依赖
- **数据库管理**：支持连接测试、引擎切换与数据迁移
- **安全认证**：JWT 令牌鉴权 + 登录 IP 限流策略
- **系统看板**：实时监控 CPU、内存、Go 协程数及长期运行状态

---

## 🗺️ 更新计划

- [x] CF Tunnel 隧道加速（创建/DNS/一键部署/启停/日志）
- [ ] CF Worker 节点一键部署
- [ ] CF ArgoX 隧道一键
- [ ] 自动优选 IP
